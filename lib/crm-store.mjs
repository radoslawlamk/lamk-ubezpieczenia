...
export const CRM_STATUSES = ['new', 'in_progress', 'offer_sent', 'awaiting_client', 'policy_concluded', 'closed_lost', 'archive'];
  ...
  return {
    ...
    addContactAction(id, action, adminId, details, ip) {
      const definitions = {
        ...
        awaiting_decision: ['Oczekiwanie na decyzję', 'awaiting_client'], policy_concluded: ['Polisa zawarta', 'policy_concluded'], client_resigned: ['Klient zrezygnował', 'closed_lost']
      };
    ...
    dashboard(options = {}) {
      ...
      const policies = db.prepare('SELECT p.*, s.form_type FROM policies p JOIN submissions s ON s.id = p.submission_id WHERE p.is_current = 1 AND s.anonymized_at IS NULL ORDER BY p.end_date').all();
    ...
    listRenewals() {
      return db.prepare(`SELECT p.*, s.form_type, s.client_email_override, s.client_name_override,
        ...
        (SELECT MAX(created_at) FROM contact_history c WHERE c.submission_id = s.id AND c.action_type = 'sms_sent') AS last_sms_at,
        (SELECT MAX(created_at) FROM contact_history c WHERE c.submission_id = s.id AND c.action_type = 'phone_call') AS last_phone_at
        FROM policies p JOIN submissions s ON s.id = p.submission_id WHERE ((p.is_current = 1 AND p.renewal_status <> 'not_due') OR p.renewal_status = 'renewed') AND s.anonymized_at IS NULL ORDER BY date(p.end_date) ASC, p.id ASC`).all().map(row => {
